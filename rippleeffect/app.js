...
import { collide } from './utils.js'; //getBWValue 
class App {
    constructor() {
    ...
    drawDots() {
        ...
        for (let i = 0; i < this.rows; i++) {
            ...
            const pixelX = Math.max(Math.min(y, this.stageHeight), 0);

            for (let j = 0; j < this.columns; j++) {
                const x = (j + 0.5) * this.pixelSize;
                const pixelY = Math.max(Math.min(x, this.stageWidth), 0);
